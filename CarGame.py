...
def GameLoop(screen):
    ...
    obstacle = Obstacle(random.randrange(0, screen.width - 100), -600, 61, 67, 4)
    ...
    while True:
        ...
        car.draw(screen)
        screen.message(["Score: " + str(score)], screen.width/5, screen.height/20)
        ...
        if car.x <= 58 or car.x + car.width >= screen.width - 58:
            screen.message(["You Crashed", "Press R", "To Restart"], screen.width/2, screen.height/3, font=font, size = 115, color=dgreen)
            crashed = True
